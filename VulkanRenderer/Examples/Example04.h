...
public:
    Ex04DescriptorSets(VulkanContext& vc, VulkanRenderer& vr) : Example(vc, vr) {
        ...
        // Material Assets (aka pipelines and pipeline layouts)
        {
            ...
            std::tie(pipeline, pipelineLayout) = vr.CreateSinglePassGraphicsPipeline(vertShader, fragShader, Vertex::GetVertexDescription(), { MeshPushConstants::GetPushConstantRange<MeshPushConstants::PushConstant2>() }, descriptorSetLayouts, vc.swapchainRenderPass);
